...
public class ReinforcementLearning {
    ...
    /**
     * Điều chỉnh điểm dựa vào lịch sử làm việc
     */
    public Chromosome adjust(Chromosome c, SchedulingContext ctx) {

        // Score cho từng nhân viên
        Map<Integer, Integer> experienceScore = new HashMap<>();

        // Lấy lịch làm trong 30 ngày gần nhất
        List<WorkSchedule> past = scheduleRepo
                .findByBranch_IdAndShiftDateBetween(
                        ctx.getBranchId(),
        ...
        for (WorkSchedule ws : past) {
            experienceScore.merge(ws.getAccount().getAccountID(), 1, Integer::sum);
        }

        c.getGenes().forEach(g -> {
            int count = experienceScore.getOrDefault(g.getStaffId(), 0);
