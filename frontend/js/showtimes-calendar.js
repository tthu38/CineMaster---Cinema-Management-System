...
/* ====================== HELPERS ====================== */
// ✅ Sửa chuẩn — tuần bắt đầu từ Thứ 2
function getMonday(d) {
    const day = d.getDay(); // 0=Sun
    const diff = (day === 0 ? -6 : 1 - day); // nếu là Chủ nhật -> lùi 6 ngày
    const m = new Date(d);
    m.setHours(0, 0, 0, 0);
...
// ✅ Sửa lỗi UTC bằng cách lấy local date thủ công
function ymd(d) {
...
async function load(keepSelectedDay = false) {
    ...
    const anchor = ymd(monday); // ✅ gửi Monday chuẩn local, không UTC
    contentArea.innerHTML = `<div class="text-center py-5 text-info">Đang tải lịch chiếu...</div>`;

    try {
        data = await showtimeApi.getWeek({ anchor, offset: 0, branchId }) ?? [];
    } catch (err) {
        console.error(err);
