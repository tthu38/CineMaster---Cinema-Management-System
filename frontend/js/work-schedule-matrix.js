// work-schedule-matrix.js
import { workScheduleApi, staffApi } from './api.js';
...
// Optional controls for editing shift hours (display-only)
...
// ===== constants =====
const SHIFTS = ['MORNING','AFTERNOON','NIGHT']; // giữ NIGHT
// màu cố định theo ID nhân viên
const PALETTE = [
    css('--p1')||'#22C1FF',
    css('--p2')||'#0AA3FF',
...
// ===== Display-only shift hours (persist per Branch via localStorage) =====
...
function renderBody(){
    ...
    tbody.innerHTML = SHIFTS.map(shift => {
        ...
        // Tiêu đề hàng: hiển thị giờ từ cấu hình FE
        return `<tr>
...
async function load(){
    ...
    renderHeader();
    tbody.innerHTML = STATE.dates.map(d => `<tr>
    <th class="sticky-col text-start">${esc(dayLabel(d))}</th>
    <td colspan="3" class="text-center muted">Loading…</td>
  </tr>`).join('');

    const H = loadDisplayHours(STATE.branchId);
    ...
    try{
        ...
        renderBody();
        renderLegend();
        try{ STAFF_CACHE = await staffApi.list(branchId); }catch{ STAFF_CACHE = []; }
    }catch(err){
        console.error(err);
        alert('Không tải được lịch (matrix). Kiểm tra API.');
    }
}
...
// ===== Edit shift hours (display) – handlers =====
if (btnEditShiftHours) {
    btnEditShiftHours.addEventListener('click', ()=>{
        ...
        // Nếu có modal trong HTML → dùng modal
        if (shiftHoursModal && HINP.mFrom && HINP.mTo && HINP.aFrom && HINP.aTo && HINP.nFrom && HINP.nTo) {
            ...
            // Fallback: dùng prompt() nếu bạn chưa thêm modal vào HTML
            const nf = prompt('Ca sáng từ ', hours.MORNING.from); if(nf===null) return;
            const nt = prompt('Đến',   hours.MORNING.to);   if(nt===null) return;
            const af = prompt('Ca chiều từ ', hours.AFTERNOON.from); if(af===null) return;
            const at = prompt('Đến',   hours.AFTERNOON.to);   if(at===null) return;
            const ef = prompt('Ca tối từ', hours.NIGHT.from); if(ef===null) return;
            const et = prompt('Đến',   hours.NIGHT.to);   if(et===null) return;

            const newHours = { MORNING:{from:nf,to:nt}, AFTERNOON:{from:af,to:at}, NIGHT:{from:ef,to:et} };
            for(const s of SHIFT_TYPES){
                const {from,to} = newHours[s];
                if(!validTime(from) || !validTime(to) || to <= from){ alert(`Giờ không hợp lệ ở ${s}`); return; }
            }
