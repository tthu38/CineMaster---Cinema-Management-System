import { workHistoryApi } from './api/workHistoryApi.js';
...
function renderTable(page) {
    const list = page?.items || page?.content || page?.data || [];
    if (!list.length) {
...
tblBody.addEventListener('click', async (e) => {
    ...
    if (btn.dataset.act === 'edit') {
        try {
            const r = await workHistoryApi.getById(id);
